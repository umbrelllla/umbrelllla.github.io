...
// some others by François Deblon
// To be used with FormElementsPlugin and HiddenSectionDataPlugin
...
// FrD le 26 novembre 2008 : ajout de 00 (pour les heures) et 00 pour les minutes
                    me.cells[d] = new Date(cy,cm,d-cd,00,00);
                }
                else {
...
// Static method to create a date from a YYYYMMDDHHMM format string
Date.convertLocalFromYYYYMMDD = function(d)
{
	return new Date(parseInt(d.substr(0,4),10),
			parseInt(d.substr(4,2),10)-1,
			parseInt(d.substr(6,2),10),
			0,0,0,0);
};

// Convert a date to local YYYYMMDDHH string format
...
if (DatePicker){
    config.macros.formElement['datechooser']= function(place,settings,tid,slot,slotvalue,refresh) {
                var startDate = slotvalue? Date.convertLocalFromYYYYMMDD(slotvalue) : null;
                var dateBox = createTiddlyElement(place,'input',null,'dateBox');         
                var dateFormat = 'YYYY/MM/DD';
                dateBox.value = startDate ? startDate.formatString(dateFormat) : '-';
                dateBox.champ=slot;
                var callback = function(el,objDate){
                    el.value = objDate.formatString(dateFormat);
// Utilisation des fonctions de HiddenSectionDataPlugin
